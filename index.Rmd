...
<font size = "4"> By Myles Walimaa and Randy Swaty </font>  


<font size = "6"> About this dashboard... </font>  
<font size = "3"> This idea was initially designed by [Randy Swaty](mailto:rswaty@tnc.org) as an alternative to a PowerPoint presentation. There was interest in building on this idea in the conservation community and I, [Myles Walimaa](mailto:mwalimaa@gmail.com), made some updates with newer data and a new charts. This can certainly be built upon and these workflows may be used for any area of interest. The State of Michigan is the target area for this dashboard, since it is where both Randy and I have lived for years.  

Randy is an ecologist for The Nature Conservancy and I am a former National Park Service employee and currently a GIS Analyst. We both work with the [Conservation Data Lab](https://conservationdatalab.org/) to mentor students in the art of computer coding and data visualization, particularly for conservation purposes.  
...
* Are there any trends that you have noticed, based off the data presented?</font>  
...
#```{r, out.width = "400px", fig.align='center', fig.cap="Coastline of Lake Superior. Photo taken by Myles Walimaa."}
#knitr::include_graphics("./dash/mi_coast.jpeg")
#```

### Area of Interest {data-commentary-width=500}
```{r}
library(tmap)
library(sf)
library(tidyverse)
...
```{r out.width = "800px", fig.align='center', fig.cap="Historical map of Michigan based on LANDFIRE's BioPhysical Settings"}
knitr::include_graphics("./dash/bps_map.jpeg")
```
...
### Historical Ecosystems {data-commentary-width=500}
```{r}
library(tidyverse)
options(scipen = 999)
...
chart2 <- ggplot(data = bps_att3, aes(fill = GROUPVEG, y = acres, x = reorder(BPS_NAME, acres))) +
  ...
  labs(title = "Top 30 Historic Ecosystems in Michigan",
       ...
       fill = "GROUPVEG") +
  theme_light() +
  scale_fill_manual(values = c("#bfbfbf", "#92c173", "#d6009c", "#4ae500", "#ceff61", "#b50000"))
...
### Ecosystems Today {data-commentary-width=500}
...
chart3 <- ggplot(data = evt_att3, aes(fill = EVT_PHYS, y = acres, x = reorder(EVT_NAME, acres))) +
  ...
  labs(title = "Top 30 Current Ecosystems in Michigan",
       ...
       fill = "EVT_PHYS") +
  theme_light() + 
  scale_fill_manual(values = c("#e2ca1b", "#92c173", "#ceff61", "#515151", "#cd7220", "#4ae500", "#b50000"))
...
### Spatial Exploration of Change {data-commentary-width=500}

```{r out.width = "800px", fig.align='center', fig.cap="Map of ecosystem change in Michigan using LANDFIRE's Succession Class dataset."}
knitr::include_graphics("./dash/change_map.jpeg")
```
...
chart <- ggplot(data = c3, aes(fill = SCL_V, y = acres, x = reorder(BPS_NAME, acres))) +
  ...
  coord_flip() +
  scale_fill_manual(values = c("#e2ca1b", "#515151", "#cd7220", "#22a601")) +
